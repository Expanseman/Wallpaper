...
def simpleRectangles(image, settings, div):
	xLeft = 0
	xRight = settings.res[0]
	yUpper = 0
	yLower = settings.res[1]
	for color in settings.palette:
		rectangle = Shape(coordinates=((xLeft, yUpper), (xRight, yLower)), color=color)
		image = paintRectangle(image=image, settings=settings, shape=rectangle)
		yUpper += div
	return image
...
def create(settings):
	...
	div = settings.res[1]/len(settings.palette)
	image = simpleRectangles(image=image, settings=settings, div=div)
	image.save('images/simpleRectangles'+str(settings.res)+'.png')
	number += 1

	div = (settings.res[1]/len(settings.palette))/2
	image = simpleRectangles(image=image, settings=settings, div=div)
	image.save('images/squeezeTop'+str(settings.res)+'.png')
	number += 1

	#image.save('out.png')
	#image.show()

def randomPalette(num):
	palette = []
	for n in range(num):
		hexColor = random.randint(0,0xFFFFFF)
		palette.append('#'+str("%06x"%hexColor))
	return palette
...
def main():
	...
	#num = 5
	#ran_col = randomPalette(num)

	wp = Settings(res=res[3], palette=blu_pin)
